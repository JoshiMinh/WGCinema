...
<div class="container-fluid px-0" style="margin-top: 80px;">
    ...
    <div class="glass-panel p-4 p-lg-5">
      <div class="row g-5">
        ...
        <div class="col-lg-3 col-md-4 text-center">
          ...
          <button class="btn btn-gradient w-100 py-3 fw-bold fs-5 text-uppercase shadow-lg d-none d-md-block" onclick="openQRCodeModal()">
        ...
        <div class="col-lg-9 col-md-8">
          ...
          <div class="d-flex flex-wrap gap-3 mb-4 text-white align-items-center">
            <span class="badge bg-primary text-white py-2 px-3 fw-bold shadow-sm" style="font-size: 0.9rem;">
              <i class="fas fa-calendar-alt me-2"></i> <span><?php echo h($movie['release_date']); ?></span>
            </span>
            <span class="badge bg-dark border border-secondary text-white py-2 px-3 fw-bold shadow-sm" style="font-size: 0.9rem;">
              Rating: <span><?php echo h($movie['age_rating'] ?? 'N/A'); ?></span>
            </span>
            <span class="badge bg-info text-dark py-2 px-3 fw-bold shadow-sm" style="font-size: 0.9rem;">
              <i class="fas fa-clock me-2"></i> <?php echo h($movie['duration']); ?> mins
            </span>
          </div>
          
          <h4 class="fw-bold gradient-text mt-5 mb-3">Synopsis</h4>
          ...
          <div class="row mb-5 text-white">
            <div class="col-md-6">
              <h5 class="fw-bold text-light mb-2">Director</h5>
          ...
          <div class="showtimes-container bg-dark bg-opacity-25 p-4 rounded-4 border border-secondary border-opacity-25">
              ...
              <div class="row g-3">
                <?php foreach ($showtimes as $st): ?>
                  <div class="col-6 col-md-4 col-lg-3">
                    <div class="showtime-card p-3 rounded-3 border border-secondary text-center cursor-pointer hover-effect"
                      onclick="selectShowtime(<?php echo $st['showtime_id']; ?>, <?php echo $st['regular_price']; ?>)">
            ...
            <?php else: ?>
              <p class="text-muted-custom">No showtimes available for this movie.</p>
            <?php endif; ?>
          </div>
          
          <button class="btn btn-gradient w-100 py-3 mt-4 fw-bold fs-5 text-uppercase shadow-lg d-block d-md-none" onclick="openQRCodeModal()">
            <i class="fas fa-ticket-alt me-2"></i> Book Now
          </button>
        </div>
      </div>
  ...
  <div style="display: none;" id="qr-code-modal" class="fancybox-content text-center">
    <h3 class="fw-bold gradient-text mb-4">Online Payment</h3>
    <p class="text-white mb-4">
      Amount: <strong class="fs-4" id="modal-price"><?php echo number_format($defaultPrice, 0, ',', '.'); ?> VND</strong>
    </p>
    ...
    <div class="d-flex justify-content-center gap-3">
      <button class="btn btn-gradient px-5 py-2 fw-bold" onclick="confirmBooking()">Confirm Transfer Completed</button>
    </div>
  </div>
</div>
...
window.selectShowtime = function(id, price) {
  ...
  $('#modal-price').text(new Intl.NumberFormat('en-US').format(price) + ' VND');
  openQRCodeModal();
};

window.confirmBooking = function() {
  if (!selectedShowtimeId) {
    Swal.fire('Notification', 'Please select a showtime first!', 'warning');
    return;
  }
  Swal.fire({
    title: 'Success!',
    text: 'Your booking request has been sent. Please check your email after we confirm the payment.',
    icon: 'success',
