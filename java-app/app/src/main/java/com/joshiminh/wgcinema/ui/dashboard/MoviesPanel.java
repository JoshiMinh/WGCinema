...
public class MoviesPanel {
    ...
    private void loadMovies() {
        ...
        try (ResultSet resultSet = MovieDAO.fetchAllMovies(url)) {
    ...
    private JPanel createMovieEntryPanel(int id, String title, String ageRating, String releaseDate) {
        ...
        editButton.addActionListener(e -> new MovieEditorDialog(url, id, false, () -> loadMovies()).setVisible(true));
    ...
    private void deleteMovie(int id) {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            int result = BaseDAO.deleteRowById(url, "movies", "id", id);
            if (result > 0) {
                loadMovies();
            } else {
    ...
    private JPanel createSearchBarPanel() {
        ...
        newMovieButton.addActionListener(e -> new MovieEditorDialog(url, -1, true, () -> loadMovies()).setVisible(true));
        ...
        searchButton.addActionListener(e -> {
            ...
            if (!query.isEmpty()) {
                new MovieSearchDialog(url, query).setVisible(true);
            } else {
    ...
    private void updateChartPanel() {
        ...
        try (ResultSet resultSet = MovieDAO.fetchAgeRatingCounts(url)) {
            while (resultSet != null && resultSet.next()) {
