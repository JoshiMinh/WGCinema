package com.joshiminh.wgcinema.ui.booking;
...
public class SeatSelectionFrame extends JFrame {
    ...
    public SeatSelectionFrame(String connectionString, int showtimeID) {
        ...
        chairsSelecting = ShowtimeDAO.getSelectingSeats(connectionString, showtimeID);
        ...
        addWindowListener(new WindowAdapter() {
            ...
            public void windowClosing(WindowEvent e) {
                ...
                System.out.println("SeatSelectionFrame window closed. User selections cleared from DB.");
            }
        });
    ...
    private void fetchMovieInfo() {
        try (ResultSet rs = MovieDAO.fetchMovieDetails(connectionString, movieId)) {
            if (rs != null && rs.next()) {
    ...
    private void setDimensions(int showroomID) {
        int totalWidth = 1450;
        try (ResultSet rs = ShowroomDAO.fetchShowroomDetails(connectionString, showroomID)) {
            if (rs != null && rs.next()) {
    ...
    private JPanel createBottomInfoPanel() {
        ...
        bookButton.addActionListener(_ -> {
            ...
            if (selectedCells.isEmpty()) {
                JOptionPane.showMessageDialog(SeatSelectionFrame.this, "Please select at least one seat.", "No Seats Selected", JOptionPane.ERROR_MESSAGE);
            } else {
                stopRefreshTimer(); // Stop refresh when going to checkout
                System.out.println("Opening Checkout. Stopping refresh timer.");
                new CheckoutDialog(connectionString, showroomID, time, movieId, date, movieTitle, movieRating, movieLink, showtimeID, selectedSeats, this);
            }
        });
    ...
    private void updateUserSeatSelection() {
        ...
        if (selectedSeats.isEmpty()) {
            ...
            // Update selection
            ShowtimeDAO.updateSeatSelection(connectionString, showtimeID, currentUserEmail, selectedSeats);
        }
    }
    ...
    private class BoxClickListener extends MouseAdapter {
        ...
        public void mouseClicked(MouseEvent e) {
            ...
            if (selectedCells.contains(box)) {
                ...
                JOptionPane.showMessageDialog(SeatSelectionFrame.this, "You have selected the maximum number of seats (" + MAX_SELECTIONS + ").", "Selection Limit Reached", JOptionPane.WARNING_MESSAGE);
    ...
    public void restartShowrooms() {
        ...
        clearUserSelectingSeats();
        dispose();
        new SeatSelectionFrame(connectionString, showtimeID).setVisible(true);
    }
}
