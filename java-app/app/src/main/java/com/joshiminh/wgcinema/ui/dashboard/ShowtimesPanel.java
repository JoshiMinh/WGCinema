...
public class ShowtimesPanel {
    ...
    public ShowtimesPanel(String url) {
        this.url = url;
        ...
        newButton.addActionListener(e -> {
            new ShowtimeEditorDialog(url, () -> refreshShowtimesPanel()).setVisible(true);
        });
        ...
        showtimesTable.getModel().addTableModelListener(e -> {
            if (e.getType() == javax.swing.event.TableModelEvent.UPDATE) {
                ...
                if (column != showtimesTable.getColumnCount() - 1) {
                    ...
                    ShowtimeDAO.updateShowtimeColumn(url, showtimesTable.getColumnName(column), updatedValue, idValue);
                }
            }
        });

        showtimesTable.getColumn("Actions").setCellRenderer(new ButtonRenderer());
        showtimesTable.getColumn("Actions").setCellEditor(
                new ButtonEditor(url, showtimesTable, "showtimes", "showtime_id", () -> refreshShowtimesPanel()));
    ...
    private void loadTableData(boolean isInitialLoad) {
        ...
        if (isInitialLoad) {
            ...
            try (ResultSet resultSet = ShowtimeDAO.fetchAllShowtimes(url)) {
                ...
                JOptionPane.showMessageDialog(null, "Error loading showtimes columns: " + e.getMessage(), "Error",
        ...
        try (ResultSet resultSet = ShowtimeDAO.fetchAllShowtimes(url)) {
            ...
            JOptionPane.showMessageDialog(null, "Error loading showtimes data rows: " + e.getMessage(), "Error",
                    JOptionPane.ERROR_MESSAGE);
        }
    }
