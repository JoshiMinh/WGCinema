...
public class MovieSearchDialog extends JFrame {
    ...
    public MovieSearchDialog(String url, String query) {
    ...
    private void refreshMoviesPanel() {
        listPanel.removeAll();
        boolean found = false;
        try (ResultSet resultSet = MovieDAO.searchMoviesByTitle(databaseUrl, currentQuery)) {
    ...
    private JButton createDeleteButton(int id, String url) {
        ...
        deleteButton.addActionListener(e -> {
            int rowsAffected = BaseDAO.deleteRowById(url, "movies", "id", id);
            if (rowsAffected > 0) {
                refreshMoviesPanel();
            } else {
    ...
    private JButton createEditButton(int id, String url) {
        ...
        editButton.addActionListener(e -> new MovieEditorDialog(url, id, false, () -> refreshMoviesPanel()).setVisible(true));
